...
import (
	...
	productErrors "github.com/aliiqbal208/GoKafka-Mongo-Observability-Stack/pkg/product_errors"
...
func (p *productMongoRepo) GetAll(ctx context.Context, pagination *utils.Pagination) (*models.ProductsList, error) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "productMongoRepo.GetAll")
	defer span.Finish()
	...
	cursor, err := collection.Find(ctx, bson.M{}, &options.FindOptions{
		Limit: &limit,
		Skip:  &skip,
		Sort:  bson.D{{Key: "createdAt", Value: -1}}, // Sort by created date, newest first
	})
	if err != nil {
